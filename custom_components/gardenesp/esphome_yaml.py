...
def _device_name(box: dict[str, Any]) -> str:
    ...
    Derived from the box **label** (``gardenesp-box-<label>``) so the resulting
    HA entity_ids are stable and branded (``switch.gardenesp_box_c_<output>``)
    and do **not** change when the free-text box name is edited. Falls back to
    the box name/id only when no label is set (legacy boxes)."""
    label = sanitize(box.get("label") or "")
    if label:
        return f"gardenesp-box-{label}".replace("_", "-")
...
def device_name(box: dict[str, Any]) -> str:
    """Public alias — the ESPHome node ``name:`` for a box (``gardenesp-box-<label>``).
...
def _friendly_name(box: dict[str, Any]) -> str:
    ...
    HA derives the box entity_ids from. Prefixed with ``GardenESP Box <Kürzel>`` so
    all box outputs/inputs share the ``gardenesp_box_<x>_*`` entity_id prefix
    (consistent with the integration's own ``gardenesp_*`` sensors) and multiple
    boxes stay distinguishable. The descriptive name is intentionally **not** part
    of it, to keep the entity_id prefix short. Falls back to ``GardenESP <name>``
    when no label is set. NB: HA freezes an entity_id at first creation — existing
    boxes keep their old ids until removed + re-added (resolver matches by entity
    name, so display is unaffected)."""
    name = box.get("name") or box.get("id") or "Box"
    label = str(box.get("label") or "").strip().upper()
    return f"GardenESP Box {label}" if label else f"GardenESP {name}"
...
def _header(box: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        "# Source: box device definition (outputs/inputs) + hw_type template (FDS §5.4).",
